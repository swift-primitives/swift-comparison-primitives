...
/// Property.View extensions for clamping operations on `Swift.Comparable` types.
///
...
extension Property.View where Base: Swift.Comparable, Tag == Comparison.Clamp {
...
extension Swift.Comparable where Self: Copyable {
    ...
    ///
    /// Returns a `Property.View` that provides clamping methods like
    /// `.between(lower, and: upper)`, `.above(minimum)`, `.below(maximum)`.
    @_disfavoredOverload
    public var clamp: Property<Comparison.Clamp, Self>.View {
        mutating _read {
            yield unsafe Property<Comparison.Clamp, Self>.View(&self)
        }
    }
}
